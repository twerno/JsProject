...
namespace jsLogic {
    ...
    export class ActionFactory {
        ...
        dispatch<T extends IActionContext, P extends IActionParam<T>>(event: ActionEvent<T, P>): DispatchEventAction<T, P> {
        ...
        randomSelector<T extends IActionContext, O>(source: IAction<T>, selectorParam: SelectorParam<O>, resultSet: O[]): RandomSelector<T, O> {
            return new RandomSelector<T, O>(source, selectorParam, resultSet);
        }


        broadcastAction<T extends IActionContext, P extends IActionParam<T>>(action2Broadcast: IActionToBroadcast<T, P>): BroadcastAction<T, P> {
