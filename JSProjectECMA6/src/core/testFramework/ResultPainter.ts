...
module testEC6 {
    ...
    export class LiveResultPainter extends IResultUpdater {

        constructor(public target: HTMLElement) { super(); }

        update(testResult: TestResult, changes: TestGroupResult | TestCaseResult | TestCaseResult[]): void {
            this.target.innerHTML = this._getHtml(testResult);
        };

        private _getHtml(testResult: TestResult): string {
        ...
        private _getGroupsHtml(testResult: TestResult): string {
            let html: string = '';
            for (let i = 0; i < testResult.groupResults.length; i++)
                html += this._getGroupHtml(testResult.groupResults[i]);

            return html;
        }

        private _getGroupHtml(groupResult: TestGroupResult): string {
            let html: string = this._getGroupHeader(groupResult);
            html += '<ul>'

            for (let i = 0; i < groupResult.testCaseResults.length; i++)
                html += `<li>${this._getTestHtml(groupResult.testCaseResults[i]) }</li>`;
        ...
        private _getTestHtml(testResult: TestCaseResult): string {
        ...
        private _getGroupHeader(groupResult: TestGroupResult): string {
            let errorMsg: string = '';
        ...
        private _getMessage(testState: TestState, errorMsg: string): string {
            if (testState === TestState.FAILURE
